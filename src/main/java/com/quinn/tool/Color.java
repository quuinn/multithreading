...
//		recolorSingleThreaded(originalImage, resultImage);
		...
		System.out.println("共花費 " + (end - start) + "毫秒");
	}
	...
	public static void recolorMultithread(BufferedImage originaImage, BufferedImage resultImage, int numberOfThreads) {
		List<Thread> threads = new ArrayList<>();
		int width = originaImage.getWidth();
		int height = originaImage.getHeight() / numberOfThreads;
		for (int i = 0; i < numberOfThreads; i++) {
			final int threadMulitiplier = i;
			Thread thread = new Thread(() -> {
				int leftCorner = 0;
				int topCorner = height * threadMulitiplier;
				recolorImage(originaImage, resultImage, leftCorner, topCorner, width, height);
			});
			threads.add(thread);
		}
